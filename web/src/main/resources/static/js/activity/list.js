...
function onFlag(value, row, index) {
    ...
    var arr = [
    	'<a class="btn btn-xs '+classType1+'" style="margin-left: 10px;" type="button" onclick="updateFlag('+ row.activityId + ',1, ' + row.flag + ')" href="javascript:void(0)">',
        '<p style="margin-bottom: 0px;">等待</p>',
        '</a>',
        '<a class="btn btn-xs '+classType2+'" style="margin-left: 10px;" type="button" onclick="updateFlag('+ row.activityId + ',2, ' + row.flag + ')" href="javascript:void(0)">',
        '<p style="margin-bottom: 0px;">通过</p>',
        '</a>',
        '<a class="btn btn-xs '+classType3+'" style="margin-left: 10px;" type="button" onclick="updateFlag('+ row.activityId + ',3, ' + row.flag + ')" href="javascript:void(0)">',
...
function onDate(value, row, index) {
	...
	if (row.startTime != null) {
		date = dateToString(row.startTime, "yyyy-MM-dd") + " - " + dateToString(row.endTime, "yyyy-MM-dd");
	}
	arr.push(date);
	return arr.join('');
}
...
function deleteById(id) {
	...
	if (r) {
		$.post("del.do", {
			...
			refreshTable("activityTable", "getActivitys");
			if (data) {
				showMsg("删除成功");
			} else {
				showErrorMsg("删除失败");
...
function lookActivity(activity) {
